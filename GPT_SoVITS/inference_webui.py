...
with open(f"./weight.json", 'r', encoding="utf-8") as file:
    ...
    print("查找的模型路径是否存在:", os.path.exists("GPT_weights_v2/白厄_extend-e20.ckpt"))
    
    # 打印调试信息
    print("gpt_path类型:", type(gpt_path))
    print("gpt_path值:", gpt_path)
    ...
    if isinstance(gpt_path, list):
        if len(gpt_path) > 0:
            ...
            # 如果列表为空，提供一个默认值或特定路径
            gpt_path = "GPT_weights_v2/白厄_extend-e20.ckpt"
            print(f"警告: gpt_path是空列表，使用默认路径: {gpt_path}")
    ...
    if isinstance(sovits_path, list):
        if len(sovits_path) > 0:
            ...
            # 如果列表为空，提供一个默认值或特定路径
            sovits_path = "SoVITS_weights_v2/白厄_extend_e12_s2664.pth"
            print(f"警告: sovits_path是空列表，使用默认路径: {sovits_path}")
...
cnhubert_base_path = os.environ.get(
    "cnhubert_base_path", "D:\Documents\白厄galgame项目\phainon-galgame\GPT_SoVITS\pretrained_models\chinese-hubert-base"
)
bert_path = os.environ.get(
    "bert_path", "D:\Documents\白厄galgame项目\phainon-galgame\GPT_SoVITS\pretrained_models\chinese-roberta-wwm-ext-large"
)
...
def change_gpt_weights(gpt_path):
    ...
    with open("./weight.json","w")as f:f.write(json.dumps(data))
    return t2s_model, dict_language
...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut=i18n("不切"), top_k=20, top_p=0.6, temperature=0.6, ref_free=False, speed=1, if_freeze=False, inp_refs=123):
    ...
    for i_text, text in enumerate(texts):
        ...
        t_phone_end = ttime()
        time_phone_bert += t_phone_end - t_phone_start

        t_infer_start = ttime()
        # 使用缓存，如果该文本段已经处理过且需要冻结
        ...
        t_infer_end = ttime()
        time_infer += t_infer_end - t_infer_start

        t_decode_start = ttime()
        refers = []
        if(inp_refs):
            for path in inp_refs:
                try:
                    ...
                    if isinstance(path, str):
                        print(f"使用参考音频文件: {path}")
                    else:
                        path = str(path)
                        print(f"已将非字符串路径转换: {path}")
                        
                    print(f"该文件存在: {os.path.exists(path)}")
                    # 使用path而非filename
            ...
            all_audio_segments.append(zero_wav)
    ...
    # 所有文本处理完毕，打印总时间统计
    print("%.3f\t%.3f\t%.3f\t%.3f" % 
          (t[0], time_phone_bert, time_infer, time_decode))
    
    # 如果有生成的音频段，合并并输出
    if all_audio_segments:
        final_audio = np.concatenate(all_audio_segments, 0)
        yield hps.data.sampling_rate, (final_audio * 32768).astype(np.int16)
    else:
        # 如果没有生成任何音频段，返回空音频
        yield hps.data.sampling_rate, np.zeros(0, dtype=np.int16)
